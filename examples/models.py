...
class TwoTank(ODE):
    ...
    def equations(self, t, x, u):
        ...
        q = u[0]  # Inputs (1): valv
        dhdt1 = q / self.F1 - self.k11 / self.F1 * self.B.core.sqrt(
            h1
        )  # self.c1 * (1.0 - valve) * pump - self.c2 * self.B.core.sqrt(h1)
        dhdt2 = (
            self.k11 / self.F2 * self.B.core.sqrt(h1)
            - self.k22 / self.F2 * self.B.core.sqrt(h2)
        )  # self.c1 * valve * pump + self.c2 * self.B.core.sqrt(h1) - self.c2 * self.B.core.sqrt(h2)
