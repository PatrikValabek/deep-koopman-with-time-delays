...
\begin{equation}
    \mathrm{argmin}_{\mathcal{K}} \|x_{k+1} - \mathcal{K}x_{k}\|_F,
\end{equation}

where \(\mathbf{X}\) and \(\mathbf{X}'\) represent the data matrices of state observations and their corresponding future states, respectively, and \( \|\cdot \| _F\) denotes the Frobenius norm. This formulation provides the basis for deriving a linear approximation of the underlying dynamics, even for systems with time delays.
...
%%MKL: toto vyzera skor ako promo na NeuroMANCER, nez ako popis implementacie. Tu treba zvyraznit, ktore funkcionality z neho vyuzivame.  @PV
...
%%MKL: x_k nieje definovane, co to reprezetuje? Ktory system?  @PV
...
\begin{subequations}
    ...
    \end{align}
\end{subequations}
%%MK: kazdy chape prediction horizon N ako pocet krokov predikcie v MPC riadeni. Je to to iste? LEbo mam pocit, ze tu je to skor pocet krokov pri trenovani Koopman modelu, cize moze vzniknut confusion.  @PV
...
%%MKL: tu mi chyba nejaky zaver, preco sa bavime of tychto loss-och? Akym sposobom to vplyva na vysledky? nejaky section-ending, ktory "namotivuje" naco som to cele cital, smerom do druhej kapitioly.  @PV
...
The experimental data were acquired by simulating this system at sampling period \( \ui{T}{s} = 10 \, \mathrm{s} \) with input delay \( \tau = 20\ui{T}{s} \). Random change in the input flow rate from the interval \( \ui{q}{\text{min}} = 0.0 \, \unit{m^3s^{-1}} \), \( \ui{q}{\text{max}} = 0.03 \) \( \unit{m^3s^{-1}} \) was applied to the system.
...
\begin{figure}[htbp]\label{fig:results:two_tanks_results}
...
%%MKL 2. Tie cisla treba naskalovat na %, t.j. z niektorej metriky treba spravit 100% resp. 1, a ostatne podla toho prepocitat. Chyba mi zaver z tejto tabulky. Co sme dosiahli?  @PV
...
\begin{figure}[htbp]\label{fig:results:two_tanks_eigs}
...
\section{Conclusion}
%%MKL: je treba v zavere spomenut v com presne nas clanok priniesol novelty a konkretne povedat, nejakou metrikou, o co sme lepsi ako nejaka ina metoda.  @PV
